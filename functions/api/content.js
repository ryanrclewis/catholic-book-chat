...
export async function onRequestGet(context) {
  ...
  try {
    const object = await bucket.get('site-content.json')
    if (object) {
    ...
    // Fallback to static content schema if R2 doesn't have custom copy yet
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Write updated content copy back to R2
    await bucket.put('site-content.json', JSON.stringify(newContent, null, 2), {
      httpMetadata: {
    ...
    return new Response(JSON.stringify({ success: true, message: 'Site content copy updated successfully' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json', ...corsHeaders() },
    })
  } catch (error) {
